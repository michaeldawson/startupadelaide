...
(function($){
    ...
    var defaults = {
        ...
        slow:     5,    // slowest spark (a higher number is slower)
    ...
    $.firefly.start = function(){
        ...
        jQuery(window).bind('resize.firefly', function(){
    ...
    $.firefly.stop = function(){
        jQuery(window).unbind('resize.firefly')
        for (i = 0; i < sparks.length; i++) jQuery(sparks[i]).stop(true)
    }
    ...
    $.firefly.remove = function() {
        for (i = 0; i < ffs.total; i++) jQuery('img[src="'+ffs.images[i]+'"]').remove()
    }
    ...
    $.firefly.create = function(i){
        var spark = jQuery('<img>').attr({'src':i}).hide()
        jQuery("body").append(spark)
